...
if __name__ == "__main__":
    ...
    for _ in range(total):
        ...
        for drug in patient.assigned_drugs:

            drug_counter[
                drug
            ] += 1

            evidence = kb.get_best_evidence(
                drug
            )
    ...
    for k, v in sorted(count_counter.items()):
    ...
    for k, v in sorted(
        ...
        key=lambda x: kb.get_evidence_rank(x[0])
    ...
    for drug, freq in drug_counter.most_common(20):
